...
@section('content')
    <div class="space-y-8 relative">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            {{-- Search Bar --}}
            <form method="GET" action="{{ route('owner.kamar') }}" id="filterForm">
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @if(($search ?? '') || ($selectedStatus ?? '') || ($selectedRoomType ?? '') || ($selectedPayment ?? '') || ($selectedFloor ?? ''))
                            <a href="{{ route('owner.kamar') }}" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-bold hover:bg-gray-200 transition flex items-center gap-1">
            ...
            <div class="flex flex-wrap items-center justify-between pt-2 border-t border-gray-100 gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="bg-gray-100 p-1 rounded-lg flex items-center">
                        <a href="{{ route('owner.kamar', array_merge(request()->query(), ['view' => 'grid'])) }}" class="p-2 rounded-md {{ request('view', 'grid') == 'grid' ? 'bg-white shadow-sm text-emerald-600' : 'text-gray-400 hover:text-gray-600' }} transition" title="Tampilan Grid">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                        </a>
                        <a href="{{ route('owner.kamar', array_merge(request()->query(), ['view' => 'list'])) }}" class="p-2 rounded-md {{ request('view') == 'list' ? 'bg-white shadow-sm text-emerald-600' : 'text-gray-400 hover:text-gray-600' }} transition" title="Tampilan List">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden flex flex-col md:flex-row h-auto group hover:shadow-md transition-all duration-200">
                                ...
                                @if($occupant)
                                    <div class="flex-1 flex items-center justify-between px-6 py-4 transition 
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div class="flex items-center gap-2">
                                                    <a href="{{ route('owner.penyewa.show', ['user' => $occupant->id, 'from' => 'kamar']) }}" class="text-base font-bold text-gray-800 hover:text-emerald-600 hover:underline">
                                                        {{ $occupant->name }}
                                                    </a>
                                                </div>
                        ...
                        <div class="w-full md:w-32 bg-gray-50 p-4 flex flex-col justify-center items-center gap-2 border-l border-gray-100">
                             ...
                             @foreach($allOccupants as $index => $occupant)
                                 <a href="{{ route('owner.penyewa.show', ['user' => $occupant->id, 'from' => 'kamar']) }}" class="w-full bg-white border border-gray-200 text-gray-600 hover:border-emerald-500 hover:text-emerald-700 text-xs font-bold py-2 rounded-lg transition text-center">
                                    @if($capacity > 1)
                                        Detail {{ chr(65 + $index) }}
                                    @else
                                        Detail
                            ...
                            <tr class="hover:bg-gray-50/50 transition bg-white">
                                             ...
                                             @foreach($allOccupants as $index => $occupant)
                                                ...
                                                <div class="flex items-center justify-between gap-4 p-2 rounded-lg bg-gray-50/50 border border-gray-100">
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        @endif
                                                        <a href="{{ route('owner.penyewa.show', ['user' => $occupant->id, 'from' => 'kamar']) }}" class="font-bold text-gray-700 hover:text-emerald-600 hover:underline text-sm">{{ $occupant->name }}</a>
                                                    </div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        @if($isOccupied)
                                            @foreach($allOccupants as $index => $occupant)
                                                <a href="{{ route('owner.penyewa.show', ['user' => $occupant->id, 'from' => 'kamar']) }}" class="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-emerald-600 hover:border-emerald-500 transition" title="Detail {{ $occupant->name }}">
