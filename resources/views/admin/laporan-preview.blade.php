...
@section('header')
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Dibuat pada {{ $report->created_at->format('d F Y H:i') }} • Oleh {{ $report->admin->name ?? 'Admin' }}</p>
        </div>
        
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.laporan') }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition font-semibold">
                ← Kembali
            </a>
            <div class="flex rounded-lg shadow-sm gap-2">
                <a href="{{ route('admin.laporan.download-pdf', $report) }}" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition font-semibold flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    PDF
                </a>
                <a href="{{ route('admin.laporan.download-excel', $report) }}" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition font-semibold flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    Excel
                </a>
            </div>
        ...
        @if ($report->report_type === 'financial_report')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-semibold">{{ $reportData['transaction_count'] }} Transaksi</span>
                </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="p-4">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-xs text-gray-500">{{ $transaction->created_at->format('H:i') }} WIB</span>
                                        </div>
                                    ...
                                    <td class="p-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-medium text-gray-900 text-sm">{{ $transaction->tenant->name ?? 'Deleted User' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-4 text-center text-xs text-gray-600">
                                        @php
                                            ...
                                            $trxPeriod = '-';
                                            if ($tenant) {
                                                 // Fallback to kamar_id from transaction if active room not set/mismatch
                                                 ...
                                                 $trxPeriod = $startDate->format('d/m/y') . ' - ' . $endDate->format('d/m/y');
                                            }
                                        @endphp
                                        {{ $trxPeriod }}
                                    </td>
                            ...
                            @foreach ($reportData['kamar'] as $room)
                                <tr class="hover:bg-gray-50">
                                            ...
                                            <div class="flex flex-col gap-1 items-center">
                                                @foreach($room->occupants as $occupant)
                                                    ...
                                                    <div class="flex items-center h-6"> <!-- Fixed height to match tenant name row -->
                                                         <div class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium {{ $payClass }}">
                                                            {{ $payLabel }}
                                                        </div>
                                                    </div>
                                                @endforeach
                                    ...
                                    <td class="p-4 text-center">
                                        ...
                                        @endphp
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $stClass }}">{{ $stLabel }}</span>
                                    </td>
                                    <td class="p-4 text-center font-medium text-gray-900">
                                        Rp {{ number_format($room->roomType->capacity > 1 ? ($room->rent_per_person ?? ($room->price_per_month/2)) : ($room->price_per_month ?? 0), 0, ',', '.') }}
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="p-4 text-center text-gray-600">
                                        @if($activeTrx)
                                             ...
                                             @endphp
                                            {{ $startDate ? $startDate->format('d/m/y') : '-' }} - {{ $activeTrx->period_end_date ? $activeTrx->period_end_date->format('d/m/y') : '-' }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td class="p-4 text-center">
                                        ...
                                        @if($payLabel == '-')
                                            <span class="text-gray-400">-</span>
                                        @else
                                            <span class="px-2 py-0.5 rounded-full text-xs font-bold {{ $payClass }}">{{ $payLabel }}</span>
                                        @endif
                                    </td>
                                    <td class="p-4 text-center">
                                        @if($tenant->account_status_label === 'Aktif')
                                             <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700">Aktif</span>
                                        @else
                                             <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-600">Tidak Aktif</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="p-4">
                            ...
                            <div class="text-xs text-gray-400 mt-1">
                                Status: <span class="{{ $tenant->activeRoom ? 'text-green-600' : 'text-red-500' }}">{{ $tenant->activeRoom ? 'Aktif' : 'Tidak Aktif' }}</span>
                            </div>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4 text-center">
                            ...
                            @endphp
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $payClass }}">
                                {{ $payLabel }}
                            </span>
                        </td>
                        <td class="p-4 text-center text-sm text-gray-600">
                            ...
                            @if($lastTrx)
                                ...
                                @endphp
                                {{ $startDate ? $startDate->format('d/m/y') : '-' }} - {{ optional($lastTrx->period_end_date)->format('d/m/y') ?? '-' }}
                            @else
                        ...
                        <td class="p-4 text-right">
                             @if($lastTrx)
                                <div class="font-medium text-gray-900">Rp {{ number_format($lastTrx->amount, 0, ',', '.') }}</div>
