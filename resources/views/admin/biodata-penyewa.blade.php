...
@section('content')
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.penyewa.biodata.edit', $penyewa->id) }}"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold rounded-xl hover:bg-emerald-100 transition shadow-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    Edit Biodata
                </a>

                <form action="{{ route('admin.penyewa.biodata-link', $penyewa->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit"
                ...
                <button type="button" onclick="openHapusPenyewaModal()"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-red-300 text-red-600 text-sm font-semibold rounded-xl hover:bg-red-50 transition shadow-sm">
    ...
    <div id="hapusPenyewaModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <p class="text-sm text-gray-500">Menghapus <span class="font-semibold text-gray-800">{{ $penyewa->name }}</span> secara <span class="font-semibold text-red-600">permanen</span> — beserta seluruh transaksi, bukti bayar, dan riwayatnya. Pemilik kos akan diberi tahu. Tindakan ini tidak bisa dibatalkan.</p>
            <form action="{{ route('admin.penyewa.destroy', $penyewa->id) }}" method="POST" class="mt-4">
                @csrf
                @method('DELETE')
